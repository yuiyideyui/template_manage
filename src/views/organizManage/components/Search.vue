  ...
  <div class="search">
    ...
    >
      <el-form-item label="组织名称">
        <el-input
          v-model.trim="searchForm.organizName"
          placeholder="请输入组织名称"
          size="small"
        />
      </el-form-item>
  ...
  methods: {
    searchTarget(formName) {
      ...
      this.searchTimer = setTimeout(() => {
        ...
        if (this.searchForm.organizName.includes('级')) {
          console.log(this.searchForm.organizName.includes('级'))
          const nameArr = this.searchForm.organizName.split('级')
          val.grade = nameArr[0]
          val.name = nameArr[1]
        } else {
          val.name = this.searchForm.organizName
        }
