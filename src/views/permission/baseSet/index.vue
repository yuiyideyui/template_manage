<template>
  <div class="app-container">
    ...
    <el-table ref="ShowMenu" v-loading="loading" :data="menuList" max-height="1000" style="width:100%;margin-top:15px" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="head-center" label="菜单路径">
        <template slot-scope="scope">
          {{ scope.row.path }}
        </template>
      </el-table-column>
      <el-table-column align="head-center" label="上级id">
        <template slot-scope="scope">
          {{ scope.row.parent_id }}
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='edit'?'编辑菜单':'新建菜单'">
      <el-form ref="menu" :model="menu" :inline="true" label-width="100px" label-position="left">
        <el-form-item hide-required-asterisk="true" label="菜单名称" prop="name">
          <el-input v-model="menu.name" placeholder="菜单名称" />
        </el-form-item>
        <el-form-item hide-required-asterisk="true" label="菜单路径" prop="path">
        ...
        <el-form-item label="上级菜单">
          <el-select v-model="menu.parent_id" class="selector" clearable placeholder="请选择,默认为最顶级">
        ...
        <el-form-item label="菜单图标">
          <el-select v-model="menu.icon" clearable filterable>
            <el-option v-for="(item,key) in iconList" v-show="item.html" :key="key" :value="item.name" :label="item.name">
              <div style="float: left;width:20px;height:20px" v-html="item.html" />
              <span style="float: right; font-size: 13px;">{{ item.name }}</span>
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="是否隐藏">
          <el-select v-model="menu.hidden" class="selector" placeholder="请选择,默认不隐藏" @change="change($event)">
            <el-option label="隐藏" value="1" />
            <el-option label="不隐藏" value="0" />
          </el-select>
        </el-form-item>
        <el-form-item label="备注">
          <el-input v-model="menu.remark" type="textarea" autosize placeholder="备注" />
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    async saveIcons() {
      ...
      for (const i in SvgIcon) {
        this.iconList.push({ name: SvgIcon[i], html: `<i class=\"el-icon-${SvgIcon[i]}\"></i>` })
      }
    },
    // 存入所有id
    async saveMenuIds(res) {
      ...
      for (const i in this.menuList) {
        this.menuIdList.push({ name: this.menuList[i].name, id: this.menuList[i].id })
      }
      this.menuTSList = []
